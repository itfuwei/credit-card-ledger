...
function showDate(date) { return date ? formatFullDate(date) : '—' }
</script>

<template>
  <section class="repayment-table">
    <div class="panel-heading"><div><h2>待还卡片</h2><p>每张卡固定一条记录，按最早到期还款日排序；已还清后自动预告下一期账单。</p></div></div>
    <el-table v-loading="loading" :data="plans" stripe empty-text="暂无还款计划">
      ...
      <el-table-column label="账单日" min-width="118"><template #default="{ row }">{{ showDate(row.statementDate) }}</template></el-table-column>
      <el-table-column label="还款日" min-width="118"><template #default="{ row }">{{ showDate(row.paymentDate) }}</template></el-table-column>
